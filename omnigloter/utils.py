...
def dist_conn_list(in_shapes, num_zones, out_size, radius, prob, weight, delay):
    ...
    for pre_pop in in_shapes:
        ...
        in_rad = radius if radius < max_dist else max_dist

        for zr in range(nrows):
            pre_r = min(in_rad + zr * radius, height - 1)
            row_l, row_h = max(0, pre_r - in_rad), min(height - 1, pre_r + in_rad)
            for zc in range(ncols):
                pre_c = min(in_rad + zc * radius, width - 1)
                col_l, col_h = max(0, pre_c - in_rad), min(width - 1, pre_c + in_rad)
